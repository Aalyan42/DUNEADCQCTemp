...
def DAT_QC(rootdir, dut_skt, duttype="FE",  env="RT") :
    while True:
        ...
        if QCresult != None:
            ...
            userinput = input ("\033[93m " + "Please contact tech coordinator  ")
            if len(userinput) > 0:
                if "139" in userinput :
                    QCstatus = "Terminate"
                    badchips = []
...
# LBL Update to ask for sig gen type:
    ...
    while True:
        ...
        pf = dat_chk_cfgfile(fcfg = pc_wrcfg_fn, duttype=duttype)
        if not pf:
            yns = input ("\033[96m Re-open config file for editing? (Y/N): " + "\033[95m" )
            if "Y" in yns or "y" in yns:
                continue
            else:
                print ("\033[91m Exit anyway \033[0m")
        ...
        # verify siggen_type is present
        siggen_found = False
        with open(pc_wrcfg_fn, "r") as f:
            for line in f:
                if line.lower().startswith("siggen_type"):
                    siggen_found = True
                    break

        if not siggen_found:
            print("\033[91m Missing 'siggen_type' entry in asic_info.csv. Please add a line like:\nsiggen_type,srs, \033[0m")
            yns = input("\033[96m Re-open config file to add it? (Y/N): " + "\033[95m")
            if "Y" in yns or "y" in yns:
                continue
            else:
                print("\033[91m Exit anyway \033[0m")
