...
if __name__ == "__main__":
    ...
    # Load the ENOB test binary file
    with open("QC_ENOB.bin", "rb") as f:
        data = pickle.load(f)

    # Look through all frequency entries
    for key in data:
        if key.startswith("enobdata_"):
            fembs, adc_data, cfg_info, waveform = data[key]
            print(f"\n🔍 Analyzing {key} ...")

            # Loop over all channels
            for ch_idx, ch_data in enumerate(adc_data):
                if not ch_data:
                    continue
                print(f" ➤ Channel {ch_idx}")
                enob, nad, sfdr, sinad, psd_dbfs, fvals = adc_enob(
                    chndata=ch_data,
                    fs=2e6,          # 2 MSPS sampling rate
                    avg_cycle=50,
                    Ntot=2**12,
                    trunc=20,
                    Vfullscale=1.4,
                    Vinput=1.3,
                    ffig=True        # Enable plotting
                )
