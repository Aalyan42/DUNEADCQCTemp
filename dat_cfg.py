...
#####################################
## LBL add to read csv to identify sig gen type
from DAT_read_cfg import dat_read_cfg

## LBL add get function operators into file

from srs_ds360 import SRS_DS360
...
###############################################################################################################
### LBL Add to read asic_info.csv

        #finding and reading csv file. and returning dictionary

        logsd, _ = dat_read_cfg(infile_mode=True, froot="./asic_info.csv")

        if "siggen_type" not in logsd:
            raise ValueError("Missing field required 'siggen_type' in asic_info.csv")

        #checks if type and port are in dictionary, stores them to variables
        self.siggen_type = logsd.get["siggen_type"].strip().lower()

        #placeholder for generator connection? where SRS360 or VISA connection will sit?
        self.gen = None
...
###############################################################################################################
        ...
        self.ADCVREF = 2.5
###############################################################################################################
### LBL will hardcoded, EACH SITE SHOULD HARCODE THEIR VALUE different sites will have differnet ports and connections
...
####################################################################################################################################
#lbl: new function for setting up SRS DS360 generator


####### AA SRS config Attempt
